...
does not replace engineering judgment. The owner-local schemas live under
`schemas/`; CLI 0.2.x implements their operational subset.
...
- `completion.required_evidence` is a non-empty list for new writers. A legacy scalar remains readable with a migration warning.
- `claim_limit` exists.
...
- Optional imported Evidence Envelope fields remain source-bounded; the checker preserves them but does not decide their semantic sufficiency.

Harness `not_proven` is an unexercised source property; Goal `not_claimed` is an explicit Goal exclusion. Neither field is automatically converted into the other.
