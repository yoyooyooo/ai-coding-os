...
This reference supports the Goal Contract branch of the `$goal-proof` experiment.

## Template

Use [`../templates/goal.yaml`](../templates/goal.yaml) as the current template.
...
completion:
  signal:
  required_evidence:
    - <one claim-bearing evidence requirement>
claim_limit:
stop_rules: []
agent_authority:
...
- completion names final proof as a list of claim-bearing evidence requirements, not just local activity;
...
- the proof-step branch can name at least one honest falsifiable path.
...
next action as proof-step discovery. Do not use `work_items` alone as readiness
evidence.
