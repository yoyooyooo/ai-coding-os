...
- 产品事实、当前术语权威或已采纳取舍。
- 任意 execution method 的运行状态和 evidence record。
- Completion evidence 或 implementation status。
- README、SSoT、standard 或 ADR 的最终正文。

## Boundary

本层是评审过程和收敛结果的记录层。它可以说明某个候选提案为什么被修改、采纳或拒绝，但不能替代目标文件本身，也不能覆盖 Product、SSoT、Standards、ADR、Architecture 或 selected execution method 的 owner。

## Promotion / Demotion

- 已达成共识的 adopted candidate 可以作为后续 README、SSoT、standard、ADR、selected execution method 或 source 修改的输入。
