...
and competing current homes. Follow only relevant edges; `AGENTS.md` and
...
identify candidate Authority, Evidence, and conflicts without loading unrelated
layers.
...
**Completion criterion:** claim class, lifecycle, Evidence level, and semantic
owner are honest and non-equivalent.
...
Preserve useful source and Evidence backlinks. Routers expose edges and inherit
...
Update affected routes and links. Run the default audit when documentation
...
audits, exceptions, unproven claims, and remaining decisions
```
