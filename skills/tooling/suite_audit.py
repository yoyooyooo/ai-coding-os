...
class Audit:
    ...
    def check_preset(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="preset-golden-") as td:
            ...
            if proc.returncode != 0:
                ...
                if (
                    ...
                    or profile_resolution.get("dependency_added") != []
                    ...
                    or "candidate" not in source_standard.lower()
        ...
        with tempfile.TemporaryDirectory(prefix="preset-negative-") as td:
            ...
            expected_minimal_terms = {"command", "command-context", "query", "use-case", "policy", "projection", "port", "repository", "transaction", "idempotency", "contract", "schema", "mapper", "public", "wiring", "composition", "migration"}
            ...
            if (
                ...
                or minimal_owners
                or minimal_terms != expected_minimal_terms
                or minimal_patterns
                ...
                or minimal_resolution.get("dependency_added") != []
                or minimal_resolution.get("resolved") != ["agent-entry", "monorepo-core"]
                or minimal_terms.intersection({"authority", "workflow", "layer", "runtime", "client", "store", "harness", "scenario"})
            ...
            if (
                ...
                or composed_owners != {"frontend-architecture", "product-harness-system"}
                or composed_pattern_owners != {"evolvable-application-architecture", "frontend-architecture"}
                or not {"monorepo-core", "typescript-node"}.issubset(composed_added)
            ...
            if (
                ...
                or not {"effect", "typescript-node", "monorepo-core"}.issubset(closure_profiles)
                or not {"effect", "typescript-node", "monorepo-core"}.issubset(closure_added)
                or closure_owners != {"effect-best-practices"}
    ...
    def check_suite_version(self) -> None:
        ...
        if not STABLE_VERSION_RE.fullmatch(version):
            self.add("error", "suite-version", "skills/VERSION must contain one stable semantic version", self.rel(version_path), version)
