...
and frontend harness rules, use `frontend-architecture`.
...
Frontend normally owns local interaction state, navigation state, drafts,
optimistic proposals, cached projections, and resource status. It normally does
not own accepted business completion, durable permissions, audit history, or
server workflow truth.
...
composition boundary. Features consume capabilities; components do not construct
transport SDKs, live runtimes, or credentials.

A frontend client that spans unrelated product capabilities can become a god
