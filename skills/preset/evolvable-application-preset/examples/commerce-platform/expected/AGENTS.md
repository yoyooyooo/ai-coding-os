...
- 事实写入、use case、外部 capability 与 host composition 保持明确边界。
- 模块、package、crate 或 deployable 本身不授予事实 Authority。
- Workspace package 只在真实编译、复用或所有权压力下晋升。
- TypeScript 点分文件名与 import 规则只属于所选生态投影。
- 远端投影只有一个 owner；本地 store 不镜像 server truth。
- Effect API 与运行时规则服从已安装 major 和声明文件。
- 优先复用现有 Harness；缺失时补最薄、可证伪的执行面。
- 区分 Proof Surface、依赖真实性、实际观察、支持结论、does-not-decide 与未证明项。
...
Preset profiles: agent-entry, application-core, monorepo-core, typescript-node, react, effect, effect-httpapi-v4, verification-core, headless-product-harness, ui-product-harness
