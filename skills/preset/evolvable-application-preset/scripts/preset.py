...
def managed_rule_lines(profile_ids: list[str], zh: bool) -> str:
    ...
    if "application-core" in selected:
        rules.extend([
            "事实写入、use case、外部 capability 与 host composition 保持明确边界。" if zh else "Keep fact writers, use cases, external capabilities, and host composition explicit.",
            "模块、package、crate 或 deployable 本身不授予事实 Authority。" if zh else "A module, package, crate, or deployable does not grant fact authority by itself.",
        ])
    if "monorepo-core" in selected:
        rules.append("Workspace package 只在真实编译、复用或所有权压力下晋升。" if zh else "Promote workspace packages only under real compilation, reuse, or ownership pressure.")
    if "typescript-node" in selected:
        rules.append("TypeScript 点分文件名与 import 规则只属于所选生态投影。" if zh else "TypeScript dot filenames and import rules belong only to the selected ecosystem projection.")
    if "rust" in selected:
        rules.append("Rust ownership、borrow、visibility 与 crate 边界不能代替产品事实 Authority、事务或授权判断。" if zh else "Rust ownership, borrowing, visibility, and crate boundaries do not replace product fact authority, transactions, or authorization decisions.")
    ...
    if "verification-core" in selected:
        rules.extend([
            ...
            "区分 Proof Surface、依赖真实性、实际观察、支持结论、does-not-decide 与未证明项。" if zh else "Separate Proof Surface, dependency reality, observations, supported conclusions, does-not-decide, and unproven neighbors.",
        ])
...
def render_architecture_profile(overlay: dict[str, Any], resolution: dict[str, list[str]]) -> str:
    ...
    if "application-core" in selected:
        decisions.update({
            "application_architecture": "authority-first",
            "backend_module_style": "private-semantic-boundary-first",
            "boundary_promotion": "pressure-driven",
            "docs_shape": "earned-semantic-layers",
            "claim_policy": "evidence-bounded",
        })
    if "monorepo-core" in selected:
        decisions["repository_mode"] = project.get("repository_mode", "monorepo")
        decisions["workspace_topology"] = "apps-packages-tooling-as-selected"
    if "typescript-node" in selected:
        decisions["typescript_source_layout"] = "bounded-semantic-flat-projection"
    if "rust" in selected:
        decisions["rust_source_layout"] = "module-tree-with-pressure-driven-crates"
        decisions["rust_async_lifecycle"] = "owned-tasks-cancellation-and-shutdown"
    if "react" in selected:
...
def render_source_standard(overlay: dict[str, Any], profiles: list[str]) -> str:
    """Render a language-aware candidate source standard.

    The shared application profile contributes semantic boundary promotion only.
    Filename and module conventions come from the selected ecosystem profiles.
    """
    zh = is_zh(overlay)
    selected = set(profiles)
    deployables = overlay.get("deployables") or []
    packages = overlay.get("packages") or []
    hosts = ", ".join(item.get("path", str(item)) if isinstance(item, dict) else str(item) for item in deployables) or "not-yet-established"
    package_text = ", ".join(item.get("path", str(item)) if isinstance(item, dict) else str(item) for item in packages) or "not-yet-established"
    ...
    lines: list[str] = [
        ...
        (
            ...
            if zh
            else "This is a Preset-generated candidate Standard. It takes effect only after the applicable owner adopts it; shared architecture semantics do not prescribe a language file layout."
        ),
        "",
        "## Semantic Boundary Promotion" if not zh else "## 语义边界晋升",
        "",
        "```text",
        ...
        (
            ...
            if zh
            else "Each promotion needs real visibility, dependency, compilation, ownership, lifecycle, trust, or deployment pressure. A directory, package, crate, or deployable does not grant fact authority."
        ),
    ]
    ...
    if "monorepo-core" in selected:
        lines.extend([
            ...
            "## Monorepo Projection",
            "",
            f"- Repository mode: `{overlay['project'].get('repository_mode', 'monorepo')}`",
            f"- Deployable hosts: {hosts}",
            f"- Workspace packages: {package_text}",
            "- apps、packages、tooling、docs 是所选仓库拓扑，不是跨语言 Doctrine。" if zh else "- apps, packages, tooling, and docs are a selected repository topology, not cross-language doctrine.",
            "- package 不得 import app internals；共享 package 不会授予事实写入权。" if zh else "- Packages do not import app internals; package sharing does not grant fact-writing authority.",
        ])
    ...
    if "typescript-node" in selected:
        ...
        lines.extend([
            ...
            "",
            "目录表达 durable ownership；文件名表达 local subject、responsibility、implementation 与 proof role。" if zh else "Directories express durable ownership; filenames express local subject, responsibility, implementation, and proof role.",
            "",
            "```text",
            "<subject>[.<facet>...].<responsibility>[.<qualifier>...].<extension>",
            "```",
            "",
            "一个 segment 内使用 kebab-case，语义维度之间使用点号。重复前缀只是 lexical cluster，不自动形成 module/package/authority。" if zh else "Use kebab-case within a segment and dots between semantic dimensions. Repeated prefixes are lexical clusters, not automatic modules, packages, or authorities.",
            "",
            ...
            "```",
            "",
            "- `*.policy.ts` 不依赖 HTTP、DB、SDK 或 live adapter。" if zh else "- `*.policy.ts` does not depend on HTTP, DB, SDK, or live adapters.",
            "- `*.use-case.ts` 不依赖 `*.live.ts` 或 transport handler。" if zh else "- `*.use-case.ts` does not depend on `*.live.ts` or transport handlers.",
            "- `*.port.ts` 不泄露 provider SDK / ORM 类型。" if zh else "- `*.port.ts` does not expose provider SDK or ORM types.",
            "- 普通业务 module 不 import `*.wiring.ts`。" if zh else "- Ordinary business modules do not import `*.wiring.ts`.",
        ])
    ...
    if "rust" in selected:
        lines.extend([
            ...
            "## Rust Projection",
            "",
            "- 使用 Rust module tree、受限 visibility 与 facade 表达私有语义边界；不要复制 TypeScript 点分文件名。" if zh else "- Use the Rust module tree, restricted visibility, and facades for private semantic boundaries; do not copy TypeScript dot filenames.",
            "- 只有独立编译、复用、所有权、公共 API 或资源生命周期压力成立时才晋升为 workspace crate。" if zh else "- Promote a workspace crate only for real compilation, reuse, ownership, public-API, or resource-lifecycle pressure.",
            "- application-owned trait 只服务真实外部 capability；普通领域协作不为可替换性幻觉制造 trait。" if zh else "- Application-owned traits serve genuine outer capabilities; ordinary domain collaboration does not need traits for hypothetical replaceability.",
            "- build-time 固定实现优先 concrete/generic composition；只有运行时选择或插件压力才承担 `dyn Trait` 与 erased async boundary。" if zh else "- Prefer concrete/generic composition for build-time-fixed implementations; accept `dyn Trait` and erased async boundaries only under runtime-selection or plugin pressure.",
            "- binary/host root 拥有 task tracking、cancellation、shutdown wait 与资源释放。" if zh else "- The binary/host root owns task tracking, cancellation, shutdown waiting, and resource release.",
            "- Serde DTO、SQL row、generated protocol type 与 authority/domain model 保持分离。" if zh else "- Keep Serde DTOs, SQL rows, generated protocol types, and authority/domain models distinct.",
            "- public API 演进需同时考虑 SemVer、MSRV、Cargo features、wire compatibility 与持久数据迁移。" if zh else "- Public API evolution considers SemVer, MSRV, Cargo features, wire compatibility, and persisted-data migration separately.",
        ])

    if "react" in selected:
        lines.extend(["", "## Frontend Projection", "", "- 远端 projection 只有一个 owner；本地 store 不镜像 server truth，host root 组装 live clients 与资源。" if zh else "- A remote projection has one owner; local stores do not mirror server truth, and host roots assemble live clients and resources."])
    if "effect" in selected:
        lines.extend(["", "## Effect Projection", "", "- Effect API、Service、Layer、Runtime 与 Scope 规则服从已安装 major 和声明文件；使用 Effect 不自动创建 package。" if zh else "- Effect API, Service, Layer, Runtime, and Scope rules follow the installed major and declarations; adopting Effect does not automatically create a package."])
    if "verification-core" in selected:
        lines.extend(["", "## Verification", "", "- Harness 不绕过正式 use-case/materialization path；静态、fake、real、restart、migration 与 production claim 保持不同上限。" if zh else "- Harnesses do not bypass formal use-case/materialization paths; static, fake, real, restart, migration, and production claims keep distinct ceilings."])
    ...
    lines.extend([
        ...
        (
            ...
            if zh
            else "`$docs-governance` owns `docs/**` layers, partitions, identities, and Current Homes. Unused layers may be omitted; stay flat by default and partition only for durable ownership, security, retention, lifecycle, reader-routing, or repeated navigation pressure."
        ),
        "",
        "## Profiles",
        "",
        *[f"- `{item}`" for item in profiles],
        "",
    ])
    return "\n".join(lines)
...
def render_vocabulary(overlay: dict[str, Any], profiles: list[str]) -> str:
    ...
    data = {
        ...
        "directory_admission": (patterns.get("directory_admission") or []) if "typescript-node" in profiles else [],
        "promotion_ladder": (patterns.get("promotion_ladder") or []) if "typescript-node" in profiles else [],
...
def inspect_repo(repo: Path) -> dict[str, Any]:
    ...
    for manifest in manifest_paths:
        try:
            ...
            continue
    locks = [n for n in ("pnpm-lock.yaml", "package-lock.json", "yarn.lock", "bun.lock", "bun.lockb", "Cargo.lock") if (root/n).exists()]
